%Epilepsy Individual Subject Connectomics: Creates individual subject DBS connectome using
%MSN's (with  images)
%   
...
%mysubject = 'subject_001';

%% 1. Define subject directory

mysubjectpath = sprintf('/home/mgh40/scratch/epilepsy/%s', mysubject);
cd(mysubjectpath); %stores all files here
mkdir epilepsy_connectome
...
system('mris_anatomical_stats -a ./FS/label/lh.aparc.annot -b /FS lh > epilepsy_connectome/lh_msn_stats.txt');
system('mris_anatomical_stats -a ./FS/label/rh.aparc.annot -b /FS rh > epilepsy_connectome/rh_msn_stats.txt');

cd epilepsy_connectome

%path to file
lh_annot_file = strcat(mysubjectpath, '/epilepsy_connectome/lh_msn_stats.txt');
rh_annot_file = strcat(mysubjectpath, '/epilepsy_connectome/rh_msn_stats.txt');
...
save('BrainNet_epilepsy.node', 'BrainNetFile', '-ascii');
save('BrainNet_epilepsy.edge', 'pearson_net_fdr_thresh', '-ascii', '-tabs'); %optional for edge weights

system('cp /home/mgh40/scratch/epilepsy/BrainNet_DBS_nodestyle.mat .');
system('cp /home/mgh40/scratch/epilepsy/BrainNet_DBS_edgestyle.mat .');

BrainNet_MapCfg('BrainMesh_Ch2_smoothed.nv', 'BrainNet_epilepsy.node', 'BrainNet_DBS_nodestyle.mat', 'BrainNet_epilepsy_node.tif'); %save image using preset parameters
BrainNet_MapCfg('BrainMesh_Ch2_smoothed.nv', 'BrainNet_epilepsy.node', 'BrainNet_epilepsy.edge', 'BrainNet_DBS_edgestyle.mat', 'BrainNet_epilepsy_edge.tif'); %save image using preset parameters
close(BrainNet);

%% Close up

filename = sprintf('epilepsy_connectome_%s%s', mysubject, '.mat');
save(filename);
